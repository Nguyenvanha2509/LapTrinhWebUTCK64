...
namespace lab1.Controllers
{
    public class LearnerController : Controller
    {
        ...
        private int pageSize = 3; // số phần tử mỗi trang
                                  // Action Index: hiển thị danh sách ban đầu
        public IActionResult Index(int? mid)
        {
            ...
            if (mid != null)
            {
                learners = learners.Where(l => l.MajorID == mid).Include(m => m.Major);
            }
        ...
        // Action LearnerFilter: phân trang + lọc + tìm kiếm
        public IActionResult LearnerFilter(int? mid, string? keyword, int? pageIndex)
        {
            var learners = db.Learners.AsQueryable();
